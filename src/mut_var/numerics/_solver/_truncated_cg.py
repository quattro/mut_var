...
from optimistix._misc import tree_dot, tree_full_like, tree_where
...
class TruncatedCG(lx.AbstractLinearSolver[_TruncatedCGState]):
    ...
    def __check_init__(self):
        ...
        if (
            ...
            raise ValueError("Must specify `rtol`, `atol`, or `max_steps` (or some combination " "of all three).")

    def init(self, operator: lx.AbstractLinearOperator, options: dict[str, Any]) -> _TruncatedCGState:
        del options
        ...
        if not eqx.tree_equal(operator.in_structure(), operator.out_structure()):
            raise ValueError("`TruncatedCG` may only be used for square linear operators.")
    ...
    def compute(
        ...
        has_scale = not (isinstance(atol, (int, float)) and isinstance(rtol, (int, float)) and atol == 0 and rtol == 0)
        if has_scale:
        ...
        class _CGState(eqx.Module):
            ...
            gamma: Scalar  # r^T r
            step: Scalar
            done: Array  # bool scalar: neg_curv or boundary exit fired
            neg_curv: Array  # bool scalar: negative curvature exit
            hit_boundary: Array  # bool scalar: trust-region boundary exit
            result_y: Any  # iterate to return at exit (y_before for early exits)
            result_d: Any  # CG direction at exit (for boundary projection)

        def cond_fun(cg_state: _CGState) -> Array:
        ...
        def body_fun(cg_state: _CGState) -> _CGState:
            ...
            if self.stabilise_every == 1:
                ...
                stable_step = (eqx.internal.unvmap_max(cg_state.step) % self.stabilise_every) == 0
            ...
            safe_gamma = jnp.where(cg_state.gamma > jnp.finfo(gamma_new.dtype).eps, cg_state.gamma, 1.0)
            ...
            tau = _find_boundary_tau(cg_state.y, cg_state.d, delta_sq, neg_curv, cg_state.gamma, inner_prod)
        ...
        if self.max_steps is None:
            result = _lxRESULTS.where(final.step == max_steps, _lxRESULTS.singular, _lxRESULTS.successful)
        elif has_scale:
    ...
    def transpose(self, state: _TruncatedCGState, options: dict[str, Any]) -> tuple[_TruncatedCGState, dict[str, Any]]:
        return state.transpose(), options

    def conj(self, state: _TruncatedCGState, options: dict[str, Any]) -> tuple[_TruncatedCGState, dict[str, Any]]:
