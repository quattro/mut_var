...
- Public pipeline API is canonicalized around package-root exports and numerics pipeline modules.
- Curve pipeline now supports explicit fit-only mode and isolates plotting side effects.
...
- Legacy internal call patterns that bypass canonical pipeline entrypoints.
...
2. Run dataframe pipelines via package-root `run_inference_pipeline` and `run_curve_pipeline`; use `mut_var.numerics.run_inference_pipeline` for numerics-level `Solution` contracts.
